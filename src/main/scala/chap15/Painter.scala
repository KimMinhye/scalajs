...
object Painter {
  ...
  @JSExport
  def createPainter(parent: html.Body, canvas: html.Canvas): Unit = {
    val title      = h2("Simple Painter").render
    val (cvs, ctx) = createCanvas(canvas)
    ...
    parent.appendChild(div(title, toolbar, cvs).render)
  }

  def createCanvas(canvas: html.Canvas): (html.Canvas, Ctx2D) = {
